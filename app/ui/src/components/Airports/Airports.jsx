...
export const Airports = ({ embedded = false }) => {
  ...
  return <section className={embedded ? 'exact-settings-airports' : 'exact-react-page'}>
    ...
    <Modal open={!!airport} title={airport?.isNew ? 'Add custom airport' : 'Edit custom airport'} onClose={()=>setAirport(null)} actions={<><button className="btn exact-secondary-action" onClick={()=>setAirport(null)}>Back</button><button className="btn primary" disabled={saveAirport.isPending || !airport?.name} onClick={()=>saveAirport.mutate()}>Save airport</button></>}>
