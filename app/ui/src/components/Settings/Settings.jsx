...
export const Settings=()=>{
  ...
  const {data,isLoading}=useSettings();const {data:customFields=[],isCustomFieldsLoading}=useCustomFields();const [settings,setSettings]=useState({});const [tab,setTab]=useState('general');const [field,setField]=useState(null);
  ...
  const tabs=[['general','General'],['previous','Previous flight experience'],['signature','Logbook signature'],['standard','Standard fields'],['custom','Custom fields']];
  return <section className="exact-react-page">
    <PageHead title="Settings" subtitle="Configure your logbook, fields, signature and previous experience." actions={tab!=='signature'&&tab!=='custom'?<button className="btn primary" disabled={save.isPending} onClick={()=>save.mutate()}>{save.isPending?'Saving…':'Save settings'}</button>:null}/>
    <Loading show={isLoading||save.isPending}/>
    <div className="settings-tabs">{tabs.map(([id,label])=><button key={id} className={tab===id?'on':''} onClick={()=>setTab(id)}>{label}</button>)}</div>
