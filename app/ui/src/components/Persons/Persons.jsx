...
export const Persons=()=>{
  ...
  return <section className="exact-react-page">
    <PageHead title="Persons" subtitle="Manage pilots, crew and other people in your logbook." actions={<button className="btn primary exact-primary-action" onClick={()=>setPerson({...blank})}>＋ Add person</button>} />
    ...
    <Modal open={!!person} title={person?.isNew?'Add person':`Edit ${personName(person)}`} onClose={()=>setPerson(null)} actions={<><button className="btn exact-secondary-action" onClick={()=>setPerson(null)}>Back</button><button className="btn primary" disabled={save.isPending} onClick={()=>save.mutate()}>Save person</button></>}>
