...
# Always use the latest Go toolchain available, and let it auto-upgrade
# further if go.mod ever requires something newer still.
...
COPY --from=go-builder --chown=weblogbook:weblogbook /out/web-logbook /web-logbook/web-logbook
